            ...
            <div class="col-12">
                <div class="card bg-dark text-light border-secondary shadow-lg w-100">
                    <div class="card-header text-warning text-center">
                        <h2>
                            <?= htmlspecialchars($filmDetails['primaryTitle']) ?>
                            (<?= htmlspecialchars($filmDetails['startYear']) ?>)</h2>
                    </div>
                    <div class="card-body">
                        <div class="mb-3 text-center">
                            <img src="<?= !empty($filmDetails['image_url']) ? $filmDetails['image_url'] : 'https://m.media-amazon.com/images/M/MV5BYzZlMjE5ZTgtNDU4Yi00NWE0LWIzN2UtZDI5OTc3ZjRiYmYyXkEyXkFqcGc@._V1_SX300.jpg' ?>" class="rounded" alt="Poster du film" style="max-width: 300px; max-height: 450px; object-fit: cover;">
                        </div>
                                        ...
                                        $directors = [];
                                            foreach ($crewMembers as $member) {
                                                if (strpos($member['primaryProfession'], 'director') !== false) {
                                                    ...
                                                    $directors[] = "<strong>" . htmlspecialchars($member['primaryName']);
                                                }
                                            }
                                            if ($directors) {
                                                foreach ($directors as $director) {
                                                    echo "<tr><td colspan='2'>$director</td></tr>";
                                                }
                                            } else {
                                    ...
                                    <td>
                                        ...
                                        foreach ($crewMembers as $member) {
                                            if (strpos($member['job'], 'screenplay') !== false) {
                                                $writer[] = htmlspecialchars($member['primaryName']);
                                            }
                                        }

                                        $writers = [];
                                            foreach ($crewMembers as $member) {
                                                if (strpos($member['primaryProfession'], 'writer') !== false) {
                                                    $characterList = $characters ? implode(", ", $characters) : 'N/A';
                                                    $writers[] = "<strong>" . htmlspecialchars($member['primaryName']);
                                                }
                                            }
                                            if ($writers) {
                                                foreach ($writers as $writer) {
                                                    echo "<tr><td colspan='2'>$writer</td></tr>";
                                                }
                                            } else {
                                                echo "<tr><td colspan='2'>N/A</td></tr>";
                                            }
